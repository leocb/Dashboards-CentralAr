...
function pegarFretes() {
  //Proteção contra DDOS deles bloquea varios pedidos simultaneos, entao usamos um intervalo de 300ms entre requisições
  GetResults(CEPArray[freteCount], freteCount);
  freteCount++
  if (freteCount < CEPArray.length)
    setTimeout(pegarFretes, 250);
}
...
function GetResults(CEP, n) {
    ...
    if (this.readyState == 4 && this.status == 200) {
      try {
        ...
        for (var i = 0; i < resposta.length; i++) {
          var vendedor = resposta[i];
          //Achei!
          if (vendedor.sellerName == "Walmart") {
            //Acha o frete "normal"
            for (var j = 0; j < resposta[i].items[0].deliveryTypes.length; j++) {
              var tiposDeEntrega = resposta[i].items[0].deliveryTypes[j];
              //Achei!
              if (tiposDeEntrega.name == "Normal CONT") {
                //Pega o valor
                valor = eval(tiposDeEntrega.price / 100).toFixed(2).replace(".", ",") ;
                //Pega o Prazo
                prazo = tiposDeEntrega.shippingEstimateInDays;
                achei = true;
                break;
              }
            }
        ...
        if (achei) {
          outText += "\n" + (n + 1) + "\tWalmart\t" + CEPText[n] + "\t" + valor + "\t" + prazo + "\tsomente um tipo";
        } else {
          outText += "\n" + (n + 1) + "\tWalmart\t" + CEPText[n] + "\t\t\tsomente um tipo";
        }
  ...
  //Para rodar o script, abra o link onde é feito a requisição e rode o script la - exemplo: https://www2.walmart.com.br/checkout/services/simulation
  
	
  xhttp.open("GET", "https://www2.walmart.com.br/checkout/services/simulation?postalCode="+CEP+"&sku=1205359&callback=jQuery20304092138050371712_1525697967065&_=1525697967068",true);
  xhttp.send();
}
