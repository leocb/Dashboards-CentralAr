...
function GetResults(CEP, n) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      try {
        //----------Saida----------
        //Pega o valor
        valor = this.responseText.match(/R\$.?\d+\,\d+/i)[0];
        //Pega o Prazo
        prazo = this.responseText.match(/\d+(?=.dias)/i)[0];
        //Monta a linha de saida final
        outText += "\n" + (n + 1) + "\tStrAr\t" + CEPText[n] + "\t" + valor + "\t" + prazo + "\tsomente um tipo";
        //verifica se já recebeu resposta de todos os fretes
        checkTotal();
      } catch (e) {
        //marca como vazio
        outText += "\n" + (n + 1) + "\tStrAr\t" + CEPText[n] + "\t\t\tsomente um tipo";
        //verifica se já recebeu resposta de todos os fretes
        checkTotal();
      }
    }
  ...
  xhttp.open("POST", "https://www.strar.com.br/productpageshipping/estimate/estimate/id/27226/", true);
  xhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
  //MODIFICAR ESSA URL, USAR O F12 NO BROWSER PRA PEGAR O PAYLOAD CORRETO PARA O PRODUTO (MENOS "CEP")
  xhttp.send('product=27225&related_product=&estimate%5Bcountry_id%5D=BR&estimate%5Bpostcode%5D=' + CEP);
}
pegarFretes();