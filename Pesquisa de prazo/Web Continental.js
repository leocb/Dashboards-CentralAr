	//Get Frete from Web continental
	...
	function GetResults(CEP, n) {
	  ...
	  xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
		  ...
		  valor = resposta.response[0].cost.replace(".", ",");
		  //Pega o Prazo
		  prazo = resposta.response[0].days;
		  //Monta a linha de saida final
		  outText += "\n" + (n + 1) + "\tWeb Continental\t" + CEP + "\t" + valor + "\t" + prazo + "\tsomente um tipo";
	  ...
	  xhttp.open("POST", "https://mid.webcontinental.com.br/frete.json", true);
	  xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	  //xhttp.setRequestHeader("Content-Encoding", "gzip");
	  xhttp.send("destino=" + CEP + "&items%5B0%5D%5Bid%5D=010101001484212221&items%5B0%5D%5Bqty%5D=1&tipooperacao=5");
