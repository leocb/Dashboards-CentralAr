...
function GetResults(CEP, n, retry = false) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      try {
        //Saida do json
        jsonAmericanas = JSON.parse(this.responseText)
          //=========================================================== Rapido

        //index do tipo que desejamos
        for (var index = 0; index < jsonAmericanas.options.length; index++) {
          var element = jsonAmericanas.options[index];
          if (element.id == "FAST") break;
          if (index == jsonAmericanas.options.length - 1) { index = 0; break; }
        }
        valor = jsonAmericanas.options[index].price.toFixed(2).replace(".", ",");
        prazo = jsonAmericanas.options[index].eta.businessDays;
        outText += "\n" + (n + 1) + "\tAmericanas\t" + CEPText[n] + "\t" + valor + "\t" + prazo + "\trapido";
        ...
        for (var index = 0; index < jsonAmericanas.options.length; index++) {
          var element = jsonAmericanas.options[index];
          if (element.id == "CONVENTIONAL") break;
          if (index == jsonAmericanas.options.length - 1) { index = 0; break; }
        }
        valor = jsonAmericanas.options[index].price.toFixed(2).replace(".", ",");
        prazo = jsonAmericanas.options[index].eta.businessDays;
        outText += "\n" + (n + 1) + "\tAmericanas\t" + CEPText[n] + "\t" + valor + "\t" + prazo + "\tnormal";

        //Pega o valor e prazo
      } catch (e) {
        console.log(e);
        outText += "\n" + (n + 1) + "\tAmericanas\t" + CEPText[n] + "\t\t\trapido";
        outText += "\n" + (n + 1) + "\tAmericanas\t" + CEPText[n] + "\t\t\tnormal";
      }
      ...
      //algum erro de rede, tente novamente
    } else if (this.readyState == 4) {
      if (!retry) {
        console.log('Erro de rede, tentando novamente');
        GetResults(CEP, n, true);
      } else {
        console.log('Erro de rede ocorreu duas vezes, verifique o link do produto.');
        //marca como vazio
        outText += "\n" + (n + 1) + "\tAmericanas\t" + CEPText[n] + "\t\t\trapido";
        outText += "\n" + (n + 1) + "\tAmericanas\t" + CEPText[n] + "\t\t\tnormal";
        checkTotal();
  ...
  xhttp.open("GET", "https://freight-v3-americanas.b2w.io/freight?c_b2wPid=1518529956526.0.2963685000167968&c_salesSolution=&cep=" + CEP + "&opn=&product=%7B%22sku%22%3A%22120881586%22%2C%22promotionedPrice%22%3A1799.99%2C%22condition%22%3A%22NEW%22%2C%22quantity%22%3A1%7D", true);
  //xhttp.open("GET", "https://b2w-region-v1.b2w.io/b2w-region/region?zipcode=" + CEP, true);
  xhttp.send();
}
pegarFretes();